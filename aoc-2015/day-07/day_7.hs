...
main = do
  ...
  let updatedAssignments = Map.insert (Wire "b") (Assignment (Wire "b") (NOP (Wire (show signalA)))) assignments
...
data Wire = Wire String deriving (Show, Eq, Ord)
...
data Assignment = Assignment Wire Op deriving (Show, Eq)
...
evaluate :: Wire -> Map.Map Wire Assignment -> Map.Map Wire Word16 -> Map.Map Wire Word16
evaluate w assignments cache
  | Map.member w cache = cache  -- w already in the cache, nothing needed.
  | all isDigit (fromWire w) = Map.insert w (read (fromWire w)) cache  -- w holds a direct number, nothing to compute, just insert.
  | otherwise = insertNew $ compute (assignments Map.! w)  -- Find the assignment, compute the related value and update the cache.
    where
      ...
      compute (Assignment (Wire _) (NOP ww)) = (c, c Map.! ww)
        where
          c = evaluate ww assignments cache
      compute (Assignment (Wire _) (AND x y)) = (c, (c Map.! x) Bits..&. (c Map.! y))
        where
          c_ = evaluate x assignments cache
          c = evaluate y assignments c_
      compute (Assignment (Wire _) (OR x y)) = (c, (c Map.! x) Bits..|. (c Map.! y))
        where
          c_ = evaluate x assignments cache
          c = evaluate y assignments c_
      compute (Assignment (Wire _) (LSHIFT x y)) = (c, Bits.shift (c Map.! x) (fromIntegral (c Map.! y) :: Int))
        where
          c_ = evaluate x assignments cache
          c = evaluate y assignments c_
      compute (Assignment (Wire _) (RSHIFT x y)) = (c, Bits.shift (c Map.! x) (negate (fromIntegral (c Map.! y) :: Int)))
        where
          c_ = evaluate x assignments cache
          c = evaluate y assignments c_
      compute (Assignment (Wire _) (NOT x)) = (c, Bits.complement $ c Map.! x)
        where
          c = evaluate x assignments cache
