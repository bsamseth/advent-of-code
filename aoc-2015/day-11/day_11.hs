...
hasStraight (a : b : c : xs) | (1, 2) == (ord b - ord a, ord c - ord a) = True
...
nextPassword s = reverse $ np $ reverse s
 where
  np :: String -> String
  np (a : b : xs) | a == 'z'  = 'a' : (np (b : xs))
                  |     -- wrap around and recurse on next character.
...
nextValidPasswords s = filter valid $ iterate nextPassword s
  where valid p = hasStraight p && twoPairs p
