...
data Pair = Pair { left :: String, right :: String } deriving (Show, Eq, Ord)
...
main = do
  ...
  putStrLn $ "Part 1: " ++ show (
    maximum [calculateHappiness order gains | order <- permutations  people])
  ...
  putStrLn $ "Part 2: " ++ show (
    maximum [calculateHappiness order gainsWithMe | order <- permutations ("Me":people)])

parseLine :: String -> (Pair, Int)
parseLine s = (Pair l r, gain)
  where
    (l:_:pm:hp:_:_:_:_:_:_:r:[]) = splitOn " " $ head $ splitOn "." s
...
calculateHappiness lst g = f (lst ++ [head lst]) g
  where
    f (x:y:xs) gains =  (gains M.! (Pair y x)) + (gains M.! (Pair x y)) + f (y:xs) gains
    f _ _ = 0
