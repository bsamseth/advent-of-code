...
main = do
  ...
  let routeLengths = [distanceOfRoute distMap perm | perm <- permutations (Set.toList locations)]
...
distanceMap parsedLines = Map.fromList $ map toMapPair parsedLines ++ map toMapPairReverse parsedLines
  where
...
allLocations ((f, t, _):xs) = Set.union (Set.fromList [f, t]) (allLocations xs)
