...
std::vector<Data> read_program(const std::string& filename)
{
    ...
    assert(in_file.good()
           && "Could not load the program file, check your working directory.");
    while (in_file.good())
    {
        char comma;
        Data token;
...
public:
    ...
    std::optional<Data> pop_if_alive(const Process<Data>& process)
    {
        std::unique_lock<std::mutex> guard(lock);
        waiting.wait(guard,
                     [this, &process] { return data.size() > 0 || !process.alive(); });
                                  ...
                                  Data relative_base = Data {0})
{
    switch (pmode)
    {
        case ParameterMode::IMMEDIATE: return read_memory(memory, ip);
        case ParameterMode::POSITION:
            return read_memory(memory, read_memory(memory, ip));
        default:
            return read_memory<Data>(memory,
                                     relative_base + read_memory<Data>(memory, ip));
    }
}
...
public:
        ...
        : inputs(std::move(in)), outputs(std::move(out))
    {
        ...
        executor = std::thread {[=]() {
            Data ip = 0;
            while (!death_requested.load()
                   && execute_inst(Opcode {read_memory(memory, ip++)}, ip))
                ;
            dead.store(true);
            outputs->notify_all();
        }};
